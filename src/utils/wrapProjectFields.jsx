...
export const wrapProjectFields = (project, className) => {
  return {
    ...
    activities: Array.isArray(project.activities) ? project.activities.map(activitySection =>
      typeof activitySection === 'string' ?
        wrapNumbersWithClass(activitySection, className) :
        {
          ...
          items: Array.isArray(activitySection.items) ? activitySection.items.map(item => wrapNumbersWithClass(item, className)) : []
        }
    ) : []
  };
};
